...
const ItemOutwards = () => {
  const [items, setItems] = useState<ItemOutwardItem[]>([]);
  ...
  const tableConfig = getItemOutwardTableConfig(translang);
  ...
  const fetchItems = async (currentPage: number) => {
    ...
    try {
      ...
      if (response.data.status) {
        ...
        setError(response.data.message || translang.failed_fetch_item_outward);
      }
    } catch (err: any) {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this?")) {
      try {
        await deleteItemOutward(id);
        fetchItems(page);
      } catch (err: any) {
        setError(err.response?.data?.message || "Failed to delete item outward");
      }
    }
  };
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <PackageMinus size={24} /> {translang.item_outward_title}
          </h1>
          <p style={{ margin: 0, color: 'var(--accents-6)' }}>{translang.item_outward_description}</p>
        </div>
        <button className="btn btn-primary" onClick={openModal} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Plus size={16} /> {translang.add_item_outward}
        </button>
      </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={tableConfig.length + 1} style={{ padding: '32px', textAlign: 'center', color: 'var(--accents-5)' }}>
                    {translang.loading_item_outward}
                  </td>
                </tr>
              ) : items.length === 0 ? (
                <tr>
                  <td colSpan={tableConfig.length + 1} style={{ padding: '32px', textAlign: 'center', color: 'var(--accents-5)' }}>
                    {translang.no_item_outward_found}
                  </td>
                </tr>
              ) : (
                items.map(item => (
