...
const AddItemInwardModal: React.FC<AddItemInwardModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const response = await storeItemInward({ 
        date: formData.date, 
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div className="card" style={{ width: '400px', padding: '24px' }}>
        <h2 style={{ marginTop: 0, marginBottom: '24px', fontSize: '18px' }}>{translang.add_item_inward_title}</h2>
        {error && (
...
export default AddItemInwardModal;
